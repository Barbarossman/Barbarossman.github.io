...
addHeader: function() {
  var header = this.createElement('h5', 'text-sm-center', 'Programming test');
...
addQuestions: function () {
  for (var i = 0; i <   questions.length; i++){
    this.addElement('ol', this.createElement('li', null, questions[i].text));
  }
},
...
addAnswers: function() {
  ...
  for (var i = 0; i ,i< li.length; i++) {
    for (var j = 0; j < questions[i].answers.length; j++) {

      var container = this.createElement('div', 'checkbox', null),
      label = this.createElement('label', null, questions[i].answers[j].text),
